...
{
    return (k == 0 ? 1 : count_arrangements(n - 1, k - 1));
}

PermList create_perm_list(int n, int k)
{
...
void destroy_perm_list(PermList pl, int n, int k)
{
    int fn = count_arrangements(n, k);
    for (int i = 0; i < fn; i++)
    {
...
// in `big_row` to `e`
void insert_and_copy(int *small_row, int size, int e, int *big_row)
{
...
void find_elements_not_in_row(int *row, int n, int k, int *elements)
{
    int c = 0;
    for (int i = 0; i < n; i++)
    {
...
PermList enumerate_arrangements(int n, int k)
{
...
void print_perm_list(PermList pl, int n, int k)
{
    int fn = count_arrangements(n, k);
    for (int i = 0; i < fn; i++)
    {
...
int main()
{
    int n = 10;
    int k = 5;
    print_perm_list(enumerate_arrangements(n, k), n, k);
    return 0;
}